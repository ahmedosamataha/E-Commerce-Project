...
To run the code
	1- make sure that MySQL is running and configured as mentioned earlier, otherwise open MySQL workbench --> server start/shutdown --> run server
	2- open the project folder with VS code
	3- in the terminal, write the command "npm install --also=dev" (without the quotes) to install the dependencies packages. 
	   (you should not need this step, as the packages exist in node_modules files, but just to be on the safe side)
	4- write either command "node app" or "npm start" (without the quotes) to run the server. (or "npm test" to run the tests)
	5- open your favourite browser and go to 'localhost:3000' to use the server
